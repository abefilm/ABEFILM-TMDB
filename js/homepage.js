document.addEventListener("DOMContentLoaded", async function () {
  ...
  if (!apiKey || !rawEndpoint) {
    console.warn("Missing API key or endpoint");
    return;
  }
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        slide.innerHTML = `
          <div class="slide-caption">
            ${logoUrl ? `<img class="logo-title" src="${logoUrl}" alt="${title} Logo" />` : `<h2>${title}</h2>`}
          </div>
        `;
  ...
  document.querySelectorAll(".widget-content").forEach((contentEl) => {
    const parentId = contentEl.closest(".widget")?.id;
    if (["HTML01", "Text1"].includes(parentId)) return;
    ...
    wrapper.dataset.endpoint = endpointPath;
    ...
    displayEl.innerHTML = [...Array(7)].map(() =>
      `<div class="tmdb-card shimmer-card">
         <div class="tmdb-thumb shimmer">
           <img src="${loaderLogoUrl}" class="tmdb-logo" alt="Logo" />
         </div>
...
const url = `https://api.themoviedb.org/3/${endpointPathClean}?api_key=${apiKey}${queryString}`;
    ...
    const scrollStep = () => {
      const card = displayEl.querySelector(".tmdb-card");
      if (!card) return 300;
    ...
    const updateButtonVisibility = () => {
      if (!wrapper.classList.contains("show-scroll-buttons")) {
        leftBtn.style.display = "none";
        rightBtn.style.display = "none";
        return;
      }
      leftBtn.style.display = displayEl.scrollLeft > 0 ? "flex" : "none";
      rightBtn.style.display = (displayEl.scrollLeft + displayEl.clientWidth) < displayEl.scrollWidth ? "flex" : "none";
    };

    wrapper.addEventListener("mouseenter", () => {
      wrapper.classList.add("show-scroll-buttons");
      updateButtonVisibility();
    });

    wrapper.addEventListener("mouseleave", () => {
      wrapper.classList.remove("show-scroll-buttons");
      updateButtonVisibility();
    });

    displayEl.addEventListener("scroll", updateButtonVisibility);
    window.addEventListener("resize", updateButtonVisibility);
    setTimeout(updateButtonVisibility, 100);

    leftBtn.onclick = () => displayEl.scrollBy({ left: -scrollStep(), behavior: "smooth" });
    rightBtn.onclick = () => displayEl.scrollBy({ left: scrollStep(), behavior: "smooth" });
  });
